...
if ! command -v docker &> /dev/null; then
    ...
    sudo apt-get update
    sudo apt-get install -y docker-ce && echo -e "${GREEN}Docker installed.${RESET}" || echo -e "${RED}Docker installation failed.${RESET}"
else
...
if ! command -v terraform &> /dev/null; then
    ...
    if [ $? -ne 0 ]; then
        echo -e "${RED}Failed to download Terraform.${RESET}"
        exit 1
    fi
